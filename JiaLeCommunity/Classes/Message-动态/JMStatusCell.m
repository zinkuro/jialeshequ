...
//    NSLog(@"imgs:============%@",model.imgs);
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"#%@# %@",model.title,model.remark]];
    ...
    [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:15] range:NSMakeRange(0, model.title.length + 3 + model.remark.length)];
    [attributedString addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, model.title.length + 3 + model.remark.length)];
...
//    self.imagesView.backgroundColor = [UIColor cyanColor];
    ...
    if (model.imgs.count > 0) {
        ...
        CGFloat width = (SCREEN_WIDTH - margin * 4) / 3.0f;
        ...
        if (model.imgs.count == 1) {
            ...
            width = 200;
            height = 200;
            imageView.frame = CGRectMake(10, 10, width, height);
            
            // 下载图片
            [imageView setImageURLStr:model.imgs[0] placeholder:placeholder];
            
            // 事件监听
            imageView.tag = TAG_NUM;
            ...
            for (int i = 0; i < 9; i++) {
                ...
                totalImagesHeight = 3 * width + 4 * margin;
            }
        }
