					...
					for (const xmlChar* key;(key=*atts++);)
					{
						...
						else if (xmlStrcasecmp(key,BAD_CAST"src")==0 && xmlStrcasecmp(name,BAD_CAST"img")==0)
						{
							...
							if (xmlStrstr(value,BAD_CAST"://")!=NULL)
							{
                                ...
                                else if (xmlStrncasecmp(value,BAD_CAST"http://",7)==0){
                                    NSString *v = [NSString stringWithUTF8String:(const char*)value];
                                    //NSLog(@"执行图片下载函数");
                                    
                                    
                                    NSData * data = [NSData dataWithContentsOfURL:[NSURL URLWithString:v]];
							...
							{
                                NSString *str3=@"http://www.jialeshequ.com";
                                NSString *v = [NSString stringWithUTF8String:(const char*)value];
                                NSLog(@"执行图片下载函数");
                                NSString *str4=[NSString stringWithFormat:@"%@%@",str3,v];
                                
                                NSData * data = [NSData dataWithContentsOfURL:[NSURL URLWithString:str4]];
						...
						else if (xmlStrcasecmp(key,BAD_CAST"href")==0)
						{
							...
							if (url.scheme==nil)
							{
								if (href.length && [[NSCharacterSet characterSetWithCharactersInString:@"+0123456789"] characterIsMember:[href characterAtIndex:0]])
                                    ...
                                    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"http://" options:NSRegularExpressionAllowCommentsAndWhitespace error:nil];
                                    //上面这段话，是集上照的是一种字符串模式，及找出<img\\ssrc[^>]*/>这样的字符出来。如果有短话就表达定义为regex。其实可以通过这段话去查找出video标签。
                                    
                                    NSArray *result = [regex matchesInString:href options:NSMatchingReportCompletion range:NSMakeRange(0, href.length)];
                                    if(result.count==0){
                                        url=[NSURL URLWithString:[@"http://www.jialeshequ.com" stringByAppendingString:href]];
                                    }
				...
				else if (xmlStrcasecmp(name,BAD_CAST"img")==0)
				{
					struct image_delegate : RunDelegateT<image_delegate>
					{
						...
						static CGFloat width(void* ref)
						{
							...
							if (width==nil)
							{
                                width=@([UIScreen mainScreen].bounds.size.width-20);//@([style[@"image"] size].width);
						...
						static CGFloat height(void* ref)
						{
							...
							if (height==nil)
							{
                                UIImage *styleImage = style[@"image"];
                                height = @(styleImage.size.height*([UIScreen mainScreen].bounds.size.width-20)/styleImage.size.width);
                                
//								height=@([style[@"image"] size].height*([UIScreen mainScreen].bounds.size.width-20)/[style[@"image"]size].width);
